...
async function main() {
  console.log('🔍 QUERYING DATABASE FOR ALL 5 WINNERS\n');
  console.log('═══════════════════════════════════════════════════════════\n');
  ...
  for (let i = 0; i < WINNER_PLAYER_IDS.length; i++) {
    ...
    if (winner) {
      ...
      console.log(`   Prize Value: ${winner.prizeValue ? JSON.stringify(winner.prizeValue) : 'N/A'}`);
      console.log('');
    }
    ...
    if (user) {
      console.log('👥 USER DATA (if registered):');
      ...
      console.log('👥 USER DATA: Not a registered user (guest player)\n');
    }
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    console.log(`| ${i + 1} | ${player.userName} | ${winner?.category || 'N/A'} | ${player.userId} | ${winner?.prizeClaimed ? 'Yes' : 'No'} |`);
  }
